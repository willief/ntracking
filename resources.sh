...
for dir in "${!dir_pid[@]}"; do
  ...
  #tcp_listening=$(lsof -n -iTCP -a -p $pid 2>/dev/null | grep "LISTEN" | wc -l)
  #echo "TCP connections (listening): $tcp_listening"

  #udp_connections=$(lsof -n -iUDP -a -p $pid 2>/dev/null | wc -l)
